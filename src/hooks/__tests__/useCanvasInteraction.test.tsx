...
describe('useCanvasInteraction', () => {
    ...
    it('should initialize with default values', () => {
        ...
        const { result } = renderHook(() => useCanvasInteraction(mockFile, false, mockRef as unknown as React.RefObject<HTMLElement>, { width: 100, height: 100 }));
    ...
    it('should not respond to events if no image is uploaded', () => {
        const mockRef = { current: { getBoundingClientRect: () => ({ left: 0, top: 0, width: 100, height: 100, x: 0, y: 0, bottom: 100, right: 100, toJSON: () => { } }) } };
        const { result } = renderHook(() => useCanvasInteraction(null, false, mockRef as unknown as React.RefObject<HTMLElement>, { width: 100, height: 100 }));

        act(() => {
            // @ts-expect-error - Mocking event
            result.current.handleWheel({ preventDefault: vi.fn(), deltaY: -100 } as React.WheelEvent);
        });
    ...
    it('should handle wheel zoom', () => {
        ...
        act(() => {
            ...
            result.current.handleWheel({
                ...
                clientX: 50,
                clientY: 50,
                currentTarget: { getBoundingClientRect: () => ({ left: 0, top: 0, width: 100, height: 100, x: 0, y: 0, bottom: 100, right: 100, toJSON: () => { } }) } as unknown as EventTarget & HTMLElement
            } as React.WheelEvent);
        });
